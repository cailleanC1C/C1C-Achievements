...
def build_config_embed(bot_version: str, config_snapshot: dict) -> discord.Embed:
    ...
    e.add_field(
        name="Loaded rows",
        value=f"Achievements: **{counts.get('ach',0)}**\nCategories: **{counts.get('cat',0)}**\nLevels: **{counts.get('lvls',0)}**",
        inline=False,
    )
    e.set_footer(text=f"Bot v{bot_version} • CoreOps v1 • {_vienna_now_str()}")
    return e
...
def build_env_embed(bot_version: str, env_info: dict) -> discord.Embed:
    """
    env_info: dict of label->string (already sanitized)
    """
...
def build_checksheet_embed(bot_version: str, status: dict) -> discord.Embed:
    """
    status:
      backend:str ('Google Sheets'|'Excel file'|'—')
      tabs:list[tuple[name:str, ok:bool, note:str]]
    """
    ...
    if status.get("tabs"):
        lines = []
        for name, ok, note in status["tabs"]:
            mark = "✅" if ok else "⚠️"
